...
function applyNonStaticProperties(baseClass, cls, options) {
    ...
    while (prot != Object.prototype) {
        for (const prop of Object.getOwnPropertyNames(prot)) {
            const doesPropertyExist = prop in baseClass || prop in baseClass.prototype;
            const canBeOverriden = !options.blacklist.nonStatic.includes(prop) || options.whitelist.nonStatic.includes(prop);

            if (doesPropertyExist && !canBeOverriden) {
                if (options.verbose) {
                    const reason = !canBeOverriden ? "blacklisted" : "already exists";
                    alt.log(`~ly~[JS] ~lb~Skipping non static property ${prop} in ${prot.constructor.name}: ${reason}`);
                }
            ...
            if (property) {
                ...
                if (options.verbose) {
                    alt.log(`~ly~[JS] ~lb~Applied non static property ${prop} in ${prot.constructor.name} to ${baseClass.name}`);
                }
            }
        }
